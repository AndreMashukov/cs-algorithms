...
function buildZArray(zString) {
  ...
  // Prefix:   ab.......
  // Z box:    .....ab..
  // Z box shift for 'a' would be 0 (0-position in prefix
  // and 0-position in Z box)
  // Z box shift for 'b' would be 1 (1-position in prefix
  // and 1-position in Z box)
  let zBoxShift = 0;
  ...
  for (let charIndex = 1; charIndex < zString.length; charIndex += 1) {
    if (charIndex > zBoxRightIndex) {
      ...
      // are are the same as the prefix. We may calculate
      // it by difference between
      ...
      if (zArray[zBoxShift] < (zBoxRightIndex - charIndex) + 1) {
        ...
        // In case if previously calculated values forces
        // us to go outside of Z box
        // we can't safely copy previously calculated zArray value.
        // It is because
        ...
        // And start comparing characters one by one
        // as we normally do for the case
...
function zAlgorithm(text, word) {
  ...
  // Based on Z-array properties each cell will tell us
  // the length of the match between
  // the string prefix and current sub-text.
  // Thus we're may find all positions in zArray
  // with the number that equals to the length of the word
  // (zString prefix) and based on
...
module.exports.zAlgorithm = zAlgorithm;
