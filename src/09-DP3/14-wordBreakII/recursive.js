/**
 * Word Break II - Recursive Solution with Memoization
 * 
...
console.log(`Result: [${wordBreak(s1, wordDict1).map(s => `"${s}"`).join(', ')}]`);
...
console.log(`Result: [${wordBreak(s2, wordDict2).map(s => `"${s}"`).join(', ')}]`);
...
console.log(`Result: [${wordBreak(s3, wordDict3).map(s => `"${s}"`).join(', ')}]`);
// Expected: []
...
console.log(`Result: [${wordBreak(s4, wordDict4).map(s => `"${s}"`).join(', ')}]`);
// Expected: ["ca rs"]
