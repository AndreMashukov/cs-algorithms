...
For an overall introduction to knapsack problem.
Function name: knapsack
Param:
  items: an array of {w: v:} (where 'w' stands for weight,
  and 'v' stands for value)
  capacity: a positive integer number
Will return max sum value that can reach, and the chosen
  subset to add up to the value.
Example:
 ...
 * in a collection so that the total weight is less than
 * or equal to a given limit and the total value is as large as possible.
 * @param {array} items - an array of {w: v:}
 * @param {number} capacity - a positive integer number
 * @return {object} - max sum value that can reach,
 * and the chosen subset to add up to the value.
 */
function knapsack(items, capacity) {
  // This implementation uses dynamic programming.
  // Variable 'memo' is a grid(2-dimentional array)
  // to store optimal solution for sub-problems,
  // which will be later used as the code execution goes on.
  // This is called memoization in programming.
  // The cell will store best solution objects
  // for different capacities and selectable items.
  const memo = [];

  // Filling the sub-problem solutions grid.
  for (let i = 0; i < items.length; i++) {
    // Variable 'cap' is the capacity for sub-problems.
    // In this example, 'cap' ranges from 1 to 6.
    const row = [];
  ...
  // The right-bottom-corner cell of the grid contains
  // the final solution for the whole problem.
  return getLast();

  /**
   * Returns the right-bottom-corner cell of the grid
   * @return {number} - right-bottom-corner cell
   */
  function getLast() {
  ...
  function getSolution(row, cap) {
    ...
    // If any one of the items weights greater than the 'cap',
    // return the last solution
    if (remaining < 0) {
      return lastSolution;
    }

    // Compare the current best solution
    // for the sub-problem with a specific capacity
