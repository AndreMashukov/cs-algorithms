/**
 * @param {array} originalSet - Original set of elements
 * we're forming power-set of.
 * @param {array} allSubsets - All subsets that have been formed so far.
 * @param {array} currentSubSet - Current subset
 * that we're forming at the moment.
 * @param {number} startAt - The position of in original set
 * we're starting to form current subset.
 * @return {array} - All subsets of original set.
 */
function btPowerSetRecursive(originalSet, allSubsets = [[]], currentSubSet = [],
  ...
  // without having duplicates. The value of startAt prevents
  ...
  for (let position = startAt; position < originalSet.length; position += 1) {
    ...
    // BACKTRACK. Exclude last element from the subset
    // and try the next valid one.
    currentSubSet.pop();
  }
...
module.exports.btPowerSet = btPowerSet;
